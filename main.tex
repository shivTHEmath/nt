...
\begin{table*}[htbp]
    ...
    \end{threeparttable}
\end{table*}
All facets of IRC and achievement see incredible gains throughout. Most notably, tranfer is predicted to  by over $2$ standard deviations, and achievement sees, on average, a $1$ standard deviation improvement. If these numbers were accurate, they would demonstrate incredible improvement, much higher than any ITS. For concreteness, a $1$ s.d. improvement takes a $50$th percentile scorer to the $83$rd percentile.
Some other trends are also visibile in the data. Contrary to expectation, initiation tendency roughly increases as we increase $e_{th}$, most likely due to the low weight attached to initiation tendency. A similar trend can be seen with transfer, which initially goes up before coming back down. Contour graphs are shown below to help visualize the effect of the engagement constraint.
\begin{figure}[H]
    \centering
...
Overall, the study weakly accomplishes the desired goal. For students at various levels, we are able to find an optimal value for performance and IRC, while making the system engaging for that student. However, the issues with noise in the data nearly invalidate the findings here. We now elaborate on the limitations that led to such noise.

\subsection{Limitations}

For one, the AI tutoring system was not used as intended by the students. Students often used the system as a mere answer checker, with students inputting the correct answer on the first try $70\%$ of the time. Thus, the parameters didn't have a huge impact on the student's development, as they never came into play.
\\\\
Another large fault that invalidated most of the metacognitive calibration data was the fact that the amount of metacognitive prompts rarely varied per student. With students inputting an answer immediately, the system rarely had the opportunity to prompt the user. On top of that, students did not answer the prompt very often\footnote{The AI system was designed to not move forward unless the student had answer the prompt or felt uncomfortable, and as a result, students who refused to answer the prompt after multiple follow-ups were often believed by the system to be ``uncomfortable" with answer the prompt.}, and thus the prompting seldom changed the metacognitive calibration of the student.
\\\\
Also, something similar could be observed on the answer specificity side. When such percents became close to $5-10\%$ of the solution, the AI was unable to give just this much away; the solutions often only consisted of 3-4 main steps. This was a result of faulty design; in future works, answer specificity will be altered to be the number of hints provided per problem, rather than the percent of the solutions given away.
\\\\
There were also a few issues with the experiment. Out of the $100$ desired participants, only $32$ actually completed $\ge 2$ assessments, making our model prone to overfitting. Furthermore, the participants interacted with the system for vastly different amounts of time, ranging from $5$ hours to $11$ hours. Also, the choice only to analyze students who completed $\ge 2$ assessments may have introduced selection bias, as those students were more motivated. This could result in offset engagement percentiles.

\subsection{Implications}

Recall that the lack of fit test demonstrated that the CCD model is likely to fit the data well. Even though the data was messed up due to a lot of noise, the ability to fit the CCD model through the data could indicate that a CCD model would adequately fit robust data as well. Thus, future work that yields strong data could utilize a similar response surface methodology approach for optimization.
\\\\
Moreover, the results in Table $\ref{tab:optimal-components}$ demonstrates that optimal parameters can have a tremendous predicted impact on achievement. These parameters would also enhance each of the components of IRC, no matter the bar for $e_{th}$. For these reasons, a teacher could theoretically implement a tutoring system with these parameters and see learning gains even in the weakest student. However, it is not recommended to do so off the current data; we recommend that a more robust study is conducted before making changes in instructional practices.

\subsection{Cost Benefit Analysis}

Here, we establish that personalized learning is a valuable investment given our results. First, consider the status quo. From $1978$ to $2025$, 9-year-old NAEP mathematics scores have improved by $.51$ standard deviations \cite{naep2025math9}, if we assume a normal distribution. Assuming a spend of $10$ billion USD yearly, this implies that nearly \textbf{half a trillion} dollars have been invested for a $.51$ standard deviation growth.
\\\\
Now if 1:1 tutoring was possible, a minimum of a $.709\sigma$ growth would be visible after at most a year; in Bloom's study, a $2\sigma$ increase only took $3$ weeks \cite{bloom1984sigma}. Assuming a token spend of $\$1$ per hour, and the tool was used $365$ hours a year, this would mean a spend of $365$ per student. Over the $50$ million students living in the States, this comes out around $18$ billion dollars, which is significantly less than the $\$500$ billion necessary for natural growth. Moreover, 1:1 tutoring is significantly faster, and allows students to reach a higher level with their education.

\subsection{Future Work}
We believe that the research here establishes a strong proof of concept but the outcomes and data contain too much noise to be implemented directly. Thus, we recommend that this study be completed again, but with the following changes:
...
\begin{itemize}
    ...
    \item The AI tutor should be fine-tuned rather than given a system prompt for robustness. This also ensure that responses follow the desired framework well.
    \item More participants be included in the study.
    \item Different parameters ranges be chosen. For this, check the limitation sections to see which ones are not suitable as is.
\end{itemize}
...
\section{Appendix E: Coefficient Tables}
...
These tests demonstrate when a certain coefficient is significantly different from $0$. If the corresponding $p$ value to a certain $t$ is $<.05$, we can conclude that that coefficient is statistically significant. A larger $t$ corresponds with a lower $p$ value. In the above tables, we see most coefficients having a very high $p$ value, demonstrating that those terms are not statistically different from $0$.
