...
\begin{document}
\title{Optimizing AI Tutoring Design for Learning and Engagement}
...
From Section $\ref{fail}$, we can conclude that a tool that both supports NLP and has a reasonable cost is necessary. LLMs satisfy both constraints --- they are designed to handle natural language, and can work at negligible costs relative to human tutors.
\\\\
...
Although this division of IRC may not be exhaustive, these components were chosen due to their measurability. The key point is that, just as standard LLMs hurt long-term knowledge retention, they negatively affect IRC as well. In Bastani's RCT, students with access to LLMs during practice sessions reported feeling more confident in their knowledge of the material compared to their peers, but performed worse than or at par with them on assessments \cite{doi:10.1073/pnas.2422633122}, demonstrating a drop in metacognitive calibration. Moreover, the same study shows that students using GPT Base, the standard GPT-4 API, began to ask for answers on the first message more frequently over the sessions, with $29\%$ of first messages asking for the answer in Session 1 and $36\%$ of first messages asking for the answer in Session 4. Although the shift is faint, it indicates that LLMs downgrade a student's initiation tendency \cite{doi:10.1073/pnas.2422633122}. Finally, depth and nuance are critical for developing transferable knowledge, and AI eliminates both in order to simplify its prose \cite{Vendrell2026Scaffolding}.
\\\\
...
We now elaborate on the settings of the four input parameters used for our CCD model and experiment. Because the coded variable is always $x_i \in \{-2, -1, 0, 1, 2\}$, we use $\xi _i$ to represent what the variable means in reality.
\begin{table}[h]
    \centering
    \begin{tabular}{c|c|c}
    \toprule
...
Answer specificity is the rough percentage of the solution given away when the student is stuck or asks for a hint. It is additive: if the previous hint gave away $p\%$ of the solution, the subsequent hint will give away $(p + \xi_1)\%$. Moreover, the answer to a problem is never given away, even if a hint is meant to give away $\ge 100\%$ of the solution; this ensures that at least minimal cognitive effort is applied. To convert from coded units to actual (so-called ``natural'') units, we use the formula
\[\xi_1 = 12x_1 +30.\] Given that the coded units are points where $x_1 \in \{-2, -1, 0, 1, 2\}$, the natural values are $\xi_1 \in \{6, 18, 30, 42, 54\}$. These specific values were chosen with the assumption that practice problems take roughly $3$ steps, which is why $x_1=0 \implies \xi_1 = 30$. One key limitation is that $30, 42,$ and $54$ correspond to $2$--$4$ hints, whereas $\xi_1 = 6$ corresponds to $16$. This is unavoidable under any mapping, since the number of hints is $100/\xi_1$, which grows rapidly. In order to enforce these parameters, the AI system was given a prompt isomorphic to
\begin{enumerate}
...
\end{enumerate}
This reveals another limitation: the exact $\xi_1$ cannot be met most of the time, though we approximate it as closely as we can without giving useless hints.
\\\\
...
\[\{-2, -1, 0, 1, 2\} \longrightarrow \{40, 53, 70, 92, 121\},\] where the values in the second set have been rounded to the nearest integer. We must also account for difficulty, since an easy problem should not receive the same access delay as a hard one. Letting difficulty be a variable $d \in \{1,\dots, 5\}$, the delays are given by
...
We make a significant decision in this section: to encapsulate performance and IRC together under a single composite learning metric. This is not exactly ideal, but it is necessary. We want students who can reason by themselves, and we want them to perform well on benchmarks; neither takes precedence over the other. Making both components of one composite outcome lets us maximize them simultaneously without giving significant preference to either. This does not contradict the $.6$ and $.4$ weighting as transfer performance is also related to achievement.
\\\\
...
$MC_{\text{init}} = |S_{1,P} - S_{1,A}|$. Students completed different numbers of assessments, as we could not enforce completion of all $5$. We therefore include only students who completed $\ge 2$, which gives us sufficient, if less than optimal, data. For students who completed 2 assessments, the final calibration is given by:
...
Entered an incorrect answer & $0.3$ or $0.8$ \\
...
\end{table}
\noindent Demonstrating thorough reasoning shows that the student has attempted the problem seriously on their own, and thus earns high points under this system. The range reflects the depth of the progress made: whether the observations were shallow or critical. Regarding the second entry, when the student asks about a specific step (e.g., ``$\texttt{how do i simplify both sides}$''), we can discern that the student has given thought to which routes might solve the problem. However, this is not as concrete as pure reasoning or work, leading to a score of $.7$. When the student enters an incorrect answer, this leaves a lot of ambiguity in the scoring: is the student guessing? Have they made a real attempt, or is this the first number that came to mind? We therefore split the difference, scoring the message $.3$ if the answer arrives within $d$ minutes, where $d \in \{1,2,3,4,5\}$ is the difficulty of the problem, and $.8$ otherwise. Generic hints demonstrate that the student would still like to attempt the problem on their own, but that they put in minimal thought before offloading the work to the AI system. The penultimate entry demonstrates offloading all cognitive work, thus receiving a score of $0$; giving up on the problem tells us that the student put no effort into it at all.
\\\\
The final initiation tendency score is the mean of these scores over all relevant problems.
\\\\
...
\textbf{Engagement.} Scoring engagement is more complicated, because we must take a student's initial engagement with mathematics into account in order to interpret their engagement with the system. A student who already enjoys math will engage more with the AI system than one who dislikes it, regardless of the parameters.
\\\\
We begin by scoring the cognitive engagement demonstrated in their messages using the ICAP framework \cite{Chi02102014}, which classifies cognitive engagement and states that
...
We average the score over all messages sent by the user to obtain the cognitive engagement score, then $z$-score it for standardization.
\\\\
...
We now compare a student's engagement with the system against what we would expect given their pre-existing engagement with mathematics. We cannot subtract directly, since the two are not necessarily on the same scale, so we fit a simple linear regression by ordinary least squares (OLS). The difference between the fitted prediction and the observed engagement is the final engagement score.
...
Because all the $p$ values are $\gg .05$, we cannot reject the null hypothesis, and thus conclude the test did not detect lack of fit. Moreover, this implies that the models' errors were not
...
\begin{itemize}
    ...
    \item However, no test detected that the RSM model did not fit the data, failing to reject the null hypothesis in the lack-of-fit test.
...
For one, the AI tutoring system was not used as intended by the students. They often used the system as a mere answer checker, inputting the correct answer on the first try $70\%$ of the time. Thus, the parameters did not have a substantial impact on the student's development, as they never came into play.
\\\\
...
\\\\
Two further faults in the system surfaced only after the study had run. The tutor treated student responses consisting of a bare integer (like $\texttt 3$) as non-mathematical, leaving it unable to reply to some answers. Separately, the tutor was quite direct at times when adhering to its parameters, causing frustration in students\footnote{as evident from the ``bug reports''}. Both would have discouraged use for reasons unrelated to the parameter setting a student was assigned, adding noise to the engagement response in particular.
\\\\
...
Recall that the lack-of-fit test demonstrated that the CCD model is likely to fit the data well. Even though the data was heavily affected by noise, the ability to fit the CCD model through it could indicate that a CCD model would adequately fit robust data as well. Thus, future work that yields strong data could utilize a similar response surface methodology approach for optimization.
\\\\
Moreover, the results in Table $\ref{tab:optimal-components}$ demonstrate that optimal parameters can have a tremendous predicted impact on achievement. These parameters would also enhance each of the components of IRC, no matter the bar for $e_{th}$. For these reasons, a teacher could theoretically implement a tutoring system with these parameters and see learning gains even in the weakest student. However, we do not recommend doing so on the basis of the current data; a more robust study should be conducted before making changes to instructional practice.
...
\\\\
Now suppose 1:1 tutoring were possible. Assuming a token spend of $\$1$ per hour, and that the tool is used $365$ hours a year, this would mean $\$365$ per student per year. Over the $50$ million students in the United States, that is roughly $\$18$ billion \emph{annually}. We take $.709\sigma$ as the gain, which is the achievement figure our model predicts at the $90$th percentile engagement threshold, that is, for the students hardest to keep engaged. We predict that gains of $.709\sigma$ are achievable in just one year; Bloom reports a $2\sigma$ increase in $3$ weeks \cite{bloom1984sigma}, so this is a conservative choice within our own results.
\\\\
We compare the two figures now on the same scale. For a $1$ standard deviation growth in performance through traditional means, we expect to require $\frac{1}{.51} \cdot 470\cdot 10^9 = 921\cdot 10^9$ in funding to reach such gains. However, with 1:1 tutoring taking $18\cdot 10^9$ annually, a $1$ standard deviation improvement will require $\frac{1}{.709}\cdot 18\cdot 10^9 = 25 \cdot 10^9$ dollars in funding. This is $37$ times less costly than the traditional route, making 1:1 tutoring both a time and cost efficient route.
\\\\
We note that this comparison rests on our own predicted $.709\sigma$, which Section \ref{sec:optimization} describes as imperfect and noisy. However, the order of magnitude remains true, and using LLMs to scale 1:1 tutoring is definitely a viable and strong approach.
...
A future study that generates less noisy data could produce predictions worth acting on, and could then follow the theory of change proposed in Figure \ref{fig:theory_of_change} toward deployment.

\subsection{Conclusion}

This study investigated whether certain parameters of an AI tutoring system --- specifically answer specificity, access delay, metacognitive prompting, and scaffolding fade --- could be adjusted to support learning and independent reasoning without sacrificing student engagement. The analysis produced promising parameter combinations and illustrated the tradeoff between deep learning and keeping students engaged. At the strictest engagement floor we tested, the fitted model predicts a $.7\sigma$ achievement gain, that is, a gain even for the students hardest to keep engaged. However, the small sample size and the minimal differences in the parameters students actually received prevent the study from identifying a true optimal configuration. The findings should therefore be viewed as a proof of concept for improving AI tutor design rather than final recommendations for classroom use.
\\\\
Future research should repeat the study with a larger sample, more consistent usage times across students, teacher-curated problems, and a tutoring system that implements each parameter more reliably. For all its limitations, this study does establish one key fact: that tradeoffs must be made between engagement and cognitive support. It is adequate proof of concept that a middle ground exists, for we were able to graph the surface, and thus that one should be looked for. Although we highly recommend against using data from this paper to influence the design of AI tutoring systems, we believe that future renditions of this study could provide parameters that perfectly achieve the desired goal.
...
\section{Appendix G: Ethics Review}

An ethics review was conducted to ensure that the study complied with local and national laws in India. It also ensured that participants of the study received no harm, and that data was deleted according to schedule.
