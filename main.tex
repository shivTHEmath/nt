...
Because all the $p$ values are $\gg .05$, we cannot reject the null hypothesis of no lack of fit. That is, the models' errors were not detectably
larger than the variation between participants who received identical
parameters. We stress that this is not positive evidence that the model fits well. With $10$ lack-of-fit and $7$ pure-error degrees of freedom, and with participant-to-participant variation as large as it was, the test has very little power; it would fail to reject against most alternatives. Read together with the negative $R^2_{\text{adj}}$ values, the honest summary is that the noise in this data is large enough to hide whatever structure may be there, in either direction.
...
\begin{itemize}
    ...
    \item The lack-of-fit test does not detect systematic misfit, but with only $7$ pure-error degrees of freedom it has little power to do so. This is an absence of evidence against the model, not evidence for it.
